...
contract FooMetaTrust23 {
    address who;
    mapping (uint256 => mapping (address => bool)) stats;

    constructor() {}
    
    function setup() external { // special caller address
    ...
    function stage1() external {
        ...
        (, bytes memory data) = msg.sender.staticcall(abi.encodeWithSignature("check()")); // caller is contract
        ...
        require(abi.decode(data, (bytes32)) == keccak256(abi.encodePacked("13337")), "stage1: !check2");  // two check func return different value
    } 
