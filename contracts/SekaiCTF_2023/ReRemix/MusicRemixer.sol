...
contract MusicRemixerSekai23 {
    ...
    constructor() payable {
        sampleEditor = new SampleEditorSekai23();
    ...
    function _getComplexity(uint256 n) internal pure returns (uint256 c) {
        // count the distinct digits in an integer
    ...
    function getSongLevel() public view returns (uint256) {
        // console.log(_getComplexity(equalizer.getGlobalInfo()));
        return convert(ud(sampleEditor.region_tempo() * 1e18).log2()) * _getComplexity(equalizer.getGlobalInfo());  
        // log2(tempo) * complexity
    }

    function finish() external {
        uint256 level = getSongLevel(); // make level >= 30
        if (level < 30)
