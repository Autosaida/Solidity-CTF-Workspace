...
contract DAOFacetSEETF23 {
    ...
    function _vote(bytes memory _sig, uint256 _proposalId, bool _support) internal {
        Proposal storage proposal = s.proposals[_proposalId];
        // check sig, bug get msg.sender's vote
        // Vulnerability: The signature verification is inadequate, 
        // resulting in a bug where the function incorrectly obtains the vote from msg.sender instead of the intended signer. 
        // This allows any address to call castVoteBySig with any signature and cast a vote on behalf of msg.sender.
