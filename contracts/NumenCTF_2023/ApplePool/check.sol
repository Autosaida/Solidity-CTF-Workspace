...
contract ApplePoolCheckNumen23 {
    ...
    constructor() public {
        ...
        IUniswapV2Pair(pair1).mint(address(this));   // pair: token0 - token1 10000:10000
        pair2 = factory.createPair(address(token1),address(token2));
        token1.transfer(pair2,10000 * 10 ** 18);
        token2.transfer(pair2,10000 * 10 ** 18);
        IUniswapV2Pair(pair2).mint(address(this));   // pair: token1 - token2 10000:10000
        ...
        appleRewardPool.addPool(IERCLike(address(token2)),starttime, endtime,0,false);  // pool: token2: 10000 token3:10000
        }
