...
contract AppleRewardPool is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePool(_pid){
        ...
        if (_pid == 0){
            ...
            IERCLike(token2).transfer(msg.sender, token2_amount);   // get token2
        }
        if (_pid == 1){
            // token2/token1*_amount = 2 * token3Number(10000)
            uint256 token3_amount = _amount.mul(rate1()).div(1e18); // token2/token1 /2
            IERCLike(token3).transfer(msg.sender, token3_amount);   // get token3
        }
        pool.token.transferFrom(_msgSender(), address(this), _amount);  // deposit token1 or token2
