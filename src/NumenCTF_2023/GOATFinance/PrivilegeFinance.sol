...
contract PrivilegeFinance { 
    ...
    function Airdrop() public {    // balance -> 1000
    ...
    function withdraw(address /*token*/, uint256 amount) public {  // balance -> 2000
    ...
    function DynamicRew(address _msgsender,uint _blocktimestamp,uint _ReferrerFees,uint _transferRate) public returns(address) {
        // set _ReferrerFees and _transferRate
        require(_blocktimestamp < 1677729610, "Time mismatch");
        require(_transferRate <= 50 && _transferRate <= 50);
        // missing verification, burnFees + _ReferrerFees should equal to _transferRate
        bytes32 _hash = keccak256(abi.encodePacked(_msgsender, rewmax, _blocktimestamp));
        address a = ecrecover(_hash, v, r, s);
        require(a == admin && time < _blocktimestamp, "time or banker");   // 1677729607 < _blocktimestamp < 1677729610
        ReferrerFees = _ReferrerFees;   // 10000000
        transferRate = _transferRate;
        return a;
    }

    function transfer(address recipient,uint256 amount) public {
        if(msg.sender == admin){   // useless
