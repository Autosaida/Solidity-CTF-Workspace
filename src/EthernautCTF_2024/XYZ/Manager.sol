...
contract Manager is Ownable(msg.sender) {
    ...
    function liquidate(address liquidatee) external {
        ...
        if (wholeDebt == totalDebt) {       // so we must borrow first 
        ...
        _updateSignals(token, protocolCollateralToken, protocolDebtToken, totalDebt);  // the only way to change signal 
    }
    ...
    function _updateDebt(IERC20 token, ERC20Signal protocolDebtToken, uint256 debtDelta, bool debtIncrease) internal {
        ...
        if (debtIncrease) {
            ...
            xyz.mint(msg.sender, debtDelta);   // the only way to get xyz
        } else {
    ...
    function _checkPosition(IERC20 token, uint256 debt, uint256 collateral) internal view {
        ...
        // collateral * price / debt >= 1.3
        // collateral * 2207 >= debt * 1.3
        // debt / collateral <= 2207 / 1.3 -> 1697.7
        // 1 sETH -> 1697.7 XYZ
    }
