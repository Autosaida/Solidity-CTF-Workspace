    ...
    ERC20("Staking Pools", "Pools")
{
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                * (accTokenPerShare[rewardTokens[i]])
                ...
                if (pending > 0) {
                    ...
                    ERC20(rewardTokens[i]).transfer(  // here: pending 
                        address(msg.sender),        
                        pending
                    );
                }
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            * (accTokenPerShare[rewardTokens[i]])
            ...
            if (pending > 0) {
                ...
                ERC20(rewardTokens[i]).transfer(address(msg.sender), pending); // here: pending
            }
        }
        if (_amount > 0) {
            user.amount = user.amount - (_amount);
    ...
    function _updatePool() internal {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            bscsReward = multiplier * (rewardPerBlock[rewardTokens[i]]);
            accTokenPerShare[rewardTokens[i]] = accTokenPerShare[  // only increase
                rewardTokens[i]
            ]
             + (
                bscsReward * (PRECISION_FACTOR[rewardTokens[i]]) /  (
