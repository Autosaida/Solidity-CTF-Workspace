...
contract Equalizer is ReentrancyGuard {
    ...
    uint[N] public gains;   // balances

    uint private constant DECIMALS = 18;
    uint public totalVolumeGain;  // totalSupply
    mapping (address => uint) public volumeGainOf;  // balanceOf
    ...
    function getGlobalInfo() external view returns (uint) {  // getVirtualPrice
        uint d = _getD(gains);
        uint _totalVolumeGain = totalVolumeGain;
        // console.log("d:", d);
        // console.log("total:", _totalVolumeGain);
        // seems always be same
     ...
     */
    function equalize(uint i, uint j, uint dx)   // swap
        external
        payable
        nonReentrant
        returns (uint dy) {
    ...
    ) payable external nonReentrant returns (uint variation) {  // addLiquidity
        uint _totalVolumeGain = totalVolumeGain;
        uint d0;
    ...
    function decreaseVolume(
        uint variation
    ) external nonReentrant returns (uint[N] memory amounts) {  // removeLiquidity
        if (variation == 0)
        ...
        for (uint i; i < N; ++i) {
            ...
            if (i == 0) {
                payable(msg.sender).sendValue(amount); // in this callback, just change the gains but the totalVolumeGain has not been changed yet
        ...
        _burn(msg.sender, variation);  // burn after sendValue
    }
}