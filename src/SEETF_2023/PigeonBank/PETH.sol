...
contract PETH is Ownable(msg.sender) {
    ...
    function flashLoan(address _userAddress, uint256 _wad, bytes calldata data) public onlyOwner {
        ...
        require(userBalanceAfter >= userBalanceBefore, "PETH: You did not return my Ether!");  // check balance, can not exploit
        ...
        if (userBalanceAfter > userBalanceBefore) {
            ...
            payable(_userAddress).sendValue(refund);     // sendValue
        }
    }
